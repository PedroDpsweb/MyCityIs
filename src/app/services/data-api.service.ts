...
export class DataApiService {
  ...
  getAll(category, ordered = false, page = false) {
    ordered == true
      ? (this.postsCollection = this.afs.collection<postInterface>(
          category,
    ...
    return (this.posts = this.postsCollection.snapshotChanges().pipe(
      map(changes => {
  ...
  getOnePost(idPost: string) {
    ...
    return (this.post = this.postDoc.snapshotChanges().pipe(
      map(action => {
  ...
  deleteComent(idPost: string, idComent, category: string) {
    this.postDoc = this.afs.doc<postInterface>(
      `${category}/${idPost}/coments/${idComent}`
    );
    this.postDoc.delete();
  }
